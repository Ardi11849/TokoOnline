<script type="text/javascript">
    ...
    function tableOrderan(from, to, type) {
        ...
        return table = $('#tOrder').DataTable({
            ...
            lengthMenu: [
                [ 10, 25, 49 ],
                [ '10 rows', '25 rows', '49 rows' ]
            ],
            buttons: [
                {extend: 'excel', footer: true, title: 'Shopee order tanggal: '+from+' - '+to}, 
                ...
                'pageLength', 
                {
                    text: 'Simpan ke DB', action: function ( e, dt, node, config ) {
                        ...
                        dt.buttons.exportData().body.forEach((item) => {
                            data.push("('<?php echo $this->session->userdata('id');?>','<?php echo $this->session->userdata('shopIdShopee');?>','"+item[2]+"','"+item[3]+"','"+item[4]+"','"+item[5]+"','Shopee','"+item[1]+"','<?php echo $this->session->userdata('id');?>',now())");
                        });
                        saveOrder(data);
                    }
                }
    ...
    function saveOrder(data) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url()?>Shopee/saveOrderShopee',
            dataType: 'json',
            data: 'data='+data,
